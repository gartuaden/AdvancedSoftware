...
void CColorConversionView::OnProblem1Sub1()
{
	...
	for (int i = 0; i < height*width; i++) {
			mg_lpImage[i] = m_dibFile.m_lpImage[i]/2;
	}
...
void CColorConversionView::OnProblem2()
{
	...
	for (int i = 0; i < height*width*3; i += 3) {
		...
		rd = m_dibFile.m_lpImage[i];
		gr = m_dibFile.m_lpImage[i+1];
		bl = m_dibFile.m_lpImage[i + 2];

		mg_lpImage[j] = 0.299*rd + 0.587*gr + 0.114*bl;
		j++;
	}
...
void CColorConversionView::OnProblem3()
{
	...
	for (int i = 0; i < height*width; i++) {
		...
		mg_lpImage[i] = (0.299*rd + 0.587*gr + 0.114*bl)/31*255;
	}
