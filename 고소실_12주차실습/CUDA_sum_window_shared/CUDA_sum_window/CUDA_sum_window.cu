...
#define BLOCK_SIZE (1 << 8)													// CUDA 커널 thread block 사이즈
...
__global__ void Sum_n_elements_Kernel_shared(IN int *d_ArrayElements, OUT int *d_SumOfArrayElements, int N, int Nf) {
	...
	if (thread_id == BLOCK_SIZE - 1) {
		for (i = BLOCK_SIZE; i < BLOCK_SIZE + Nf; i++) {
			if (id + i + 1 - BLOCK_SIZE > N - 1) shared_buffer[Nf + i] = 0;
			else shared_buffer[Nf + i] = d_ArrayElements[id + i + 1 - BLOCK_SIZE];
		}
	}
	...
	for (i = -Nf; i <= Nf; i++) {
		sum += shared_buffer[thread_id + Nf + i];
	}
